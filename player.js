...
function animateNextCard(newIdx, onDone){
  ...
  // Подготавливаем ghost как новую карточку — падает сверху
  ghostCard._idx=-1;
  fillSideCard(ghostCard,newIdx);
  ghostCard.style.transition='none';
...
function carouselReset(){
  ...
  // fillSideCard имеет кэш el._idx — не перерисует если индекс совпадает
  // При движении назад в шафле prevCard уже заполнена правильно — не трогаем
  fillSideCard(prevCard, leftIdx);
  fillSideCard(nextCard, rightIdx);
  snapCard(prevCard,-CAROUSEL_OFFSET,CAROUSEL_SCALE,CAROUSEL_DIM);
  snapCard(playerCard,0,1,1);
  snapCard(nextCard,CAROUSEL_OFFSET,CAROUSEL_SCALE,CAROUSEL_DIM);
  ghostCard.style.transition='none'; ghostCard.style.opacity='0';
  ghostCard.style.transform='translateX(9999px)'; ghostCard.style.zIndex='';
}
...
function carouselNavigate(dir, onSwap){
  ...
  // При шафле явно предзагружаем нужные карточки
  if (isShuffle){
    if (dir > 0 && _shuffleQueue.length > 0){
      var s0 = TRACKS[_shuffleQueue[0]]; if (s0&&s0.cover) { var i0=new Image(); i0.src=s0.cover; }
      if (_shuffleQueue.length > 1){ var s1=TRACKS[_shuffleQueue[1]]; if(s1&&s1.cover){var i1=new Image();i1.src=s1.cover;} }
    } else if (dir < 0 && _history.length > 0){
      var h0=TRACKS[_history[_history.length-1]]; if(h0&&h0.cover){var ih0=new Image();ih0.src=h0.cover;}
      if (_history.length>1){var h1=TRACKS[_history[_history.length-2]];if(h1&&h1.cover){var ih1=new Image();ih1.src=h1.cover;}}
    }
  }
